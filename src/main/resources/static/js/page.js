var TODO = (function(window) {
	...
	var list_html = "<div class='list_wrapper'>"
			...
			+ "</div>" + "<a class='add_card' href='#''>Add a Card...</a>"
			+ "</div>" + "</div>" + "</div>"
	...
	var comment_html = "<div class='comment'>"
			...
			+ "<div class='comment_reply'> Reply</div>" + "</div>";
	...
	function show_modal(e) {
		...
		var list_name = $(e.target).closest(".list_content").find(
				".list_header_name").val();
		$(".list_name").text(list_name);
	}

	function card_cancel(e) {

		$(e.target).closest(".card_composer .add_card_form").css('display',
				'none');
		$(e.target).closest(".card_composer").find("a.add_card").css('display',
				'block');
	}
